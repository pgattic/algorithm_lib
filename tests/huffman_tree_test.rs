...
mod huffman_tree_test {
    ...
    fn test_encode() {
        let text = "the rain in spain stays mainly in the plain";
        let tree = huffman_tree::build_tree(&text);
        let enc_map = &huffman_tree::create_encoding_map(&tree);
        let encoded = huffman_tree::encode(text, enc_map).unwrap();
    ...
    fn test_encode_decode() {
        ...
        let decoded = huffman_tree::decode(&encoded, &tree).unwrap();
